...
def get_candidates(token, replacement_types, max_results=50):
    ...
    return [
        o
        for o in options
        if "tags" in o and spacyPOS_to_datamusePOS(token.pos_) in o["tags"]
    ]
...
def replace_marked_words(filename, replacement_types=["ml"], max_options=50):
    ...
    for i, token in enumerate(tokens):
        if i >= 2 and tokens[i - 1].text == "BEGIN" and tokens[i - 2].text == "#":
            ...
            readingPoemContent = False
        toReplace = (
            i >= 2 and tokens[i - 1].text == "REPLACE" and tokens[i - 2].text == "#"
        )
        newWord = token.text
...
def replace_random_words(
    filename, percent_to_replace, replacement_types=["ml"], max_options=50
):
    start = time.time()
    data = read_poem(filename)
    ...
    outFilename = filename.replace(
        ".txt", f"_{percent_to_replace}_{'_'.join(replacement_types)}_{curDatetime}.txt"
    )
    print(outFilename)
    save_poem(out_words, outFilename)
    ...
    print(
        f"Experiment time elapsed ({outFilename}): {end - start} seconds ({(end - start) / 60} minutes)"
    )
...
def run_experiments_on_poem():
    ...
    args = [
        (filename, percent, repl)
        for percent in percent_options
        for repl in replacement_options
    ]
...
def replace_all_random_words(
    authorName, idx, tokens, content_tokens, replacement_types=["ml"], max_options=50
):
    start = time.time()
    ...
    for i, token in enumerate(tokens):
        ...
        if readingPoemContent and token in content_tokens:
            ...
            if len(options) <= 1:
                print(
                    f"Num candidates: {len(options)}, word: {token.text.strip(punctuation)}, type: {','.join(replacement_types)}"
                )
            if len(options) > 0:
                chosen = sample(options, 1)
                newWord = f"{chosen[0]['word']}[#ORIGINAL_{token.text}]"
    ...
    curDatetime = datetime.datetime.now().strftime("%Y-%m-%d-%H.%M.%S.%f")
    outFilename = (
        f"{authorName}-{idx}_100_{'_'.join(replacement_types)}_{curDatetime}.txt"
    )
    print(outFilename)
    save_poem(out_words, outFilename)
    ...
    print(
        f"\t\tPoem variation time elapsed ({outFilename}): {end - start} seconds ({(end - start) / 60} minutes)"
    )
...
def run_author_poem(authorName, poem, idx):
    ...
    args = [
        (authorName, idx, tokens, content_tokens, repl) for repl in replacement_options
    ]
    ...
    print(
        f"\tDone {authorName} poem {idx} variations: {end - start} seconds ({(end - start) / 60} minutes)"
    )
...
def run_author_poem_ai(authorName, poem, idx):
    ...
    for line in originalLines:
        PROMPT = f"{line} \n\n###\n\n"
        res = ai.completions.create(model=FINE_TUNED_MODEL, prompt=PROMPT)
...
def generatePoem(text):
    ...
    curDatetime = datetime.datetime.now().strftime("%Y-%m-%d-%H.%M.%S.%f")
    outFilename = (
        f"_{percent_to_replace}_{'_'.join(replacement_types)}_{curDatetime}.txt"
    )
    print(outFilename)
...
def generateNVariations(text, n, replacement_types=["ml"]):
    ...
    for i, token in enumerate(tokens):
        ...
        if token in to_replace:
            ...
            if len(options) > 0:
                ...
                newWords = [
                    f"{chosenWords[i % numChosen]['word']}[#ORIGINAL_{token.text}]"
                    for i in range(n)
                ]
            else:
    ...
    print(
        f"Generated {n} variations in: {end - start} seconds ({(end - start) / 60} minutes)"
    )
...
def generateNVariations_parallel(
    nlp, text, nVars, replacement_types=["ml"], save=False, filenameBase=""
):
    start = time.time()
    doc = nlp(text)
    tokens = [token for token in doc]
    ...
    for p in range(nVars):
        if save == True:
            curDatetime = datetime.datetime.now().strftime("%Y-%m-%d-%H.%M.%S.%f")
            outFilename = (
                f"{filenameBase}_{'_'.join(replacement_types)}_{curDatetime}.txt"
            )
            print(outFilename)
    ...
    print(
        f"Generated {nVars} variations in: {end - start} seconds ({(end - start) / 60} minutes)"
    )
...
def generateVariationsFromFile(
    filename="poems/original/modernists_ascii.txt",
    nVarsPerType=1,
    replacementTypes=["ml"],
    outputPath="poems/generated",
):
    start = time.time()
    nlp = spacy.load("en_core_web_md")
    data = read_poem(filename)
    ...
    def generateVariationsForAuthor(authorText):
        ...
        args = [
            (
                ...
                replacementTypes,
                True,
                f"{outputPath}/{authorName}-{i}",
            )
            for i in range(1, len(poems))
        ]
        ...
        print(
            f"DONE {authorName}: {end - start} seconds ({(end - start) / 60} minutes)"
        )
...
if __name__ == "__main__":
    ...
    generateVariationsFromFile(
        filename="poems/original/2024-06-19_ascii.txt",
        nVarsPerType=1,
        replacementTypes=["rel_hom"],
        outputPath="poems/generated/direct-replace/2024-06-19",
    )
