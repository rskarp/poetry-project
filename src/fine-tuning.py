...
def fineTune1():
    ...
    for poem in lines:
        ...
        poemFilePath = os.path.join(
            dirname, f"poems/generated/direct-replace/{poemFilename}"
        )
        try:
        ...
        for label in good_labels:
            labeledLine = poem[
                label["location"]["start"] : label["location"]["end"] + 1
            ].replace("\n", "\\n")
    ...
    outFilePath = os.path.join(
        dirname, "poems/labels/modernist-labels-good-training.jsonl"
    )
...
def fineTune2():
    ...
    for poem in lines:
        ...
        for label in labels:
            ...
            trainingLine = (
                "{"
                + f'"prompt": "<original>{original}</original> : <generated>{generated}</generated>\\n\\n###\\n\\n", "completion": " {label["name"]}\\n"'
                + "}"
            )
            # print(trainingLine)
    ...
    outFilePath = os.path.join(
        dirname, "poems/labels/ft2-modernist-labels-training_5000.jsonl"
    )
    fout = open(outFilePath, "x")
...
def _process_line(line):
    ...
    prompt = original_data["prompt"].strip().split("###")[0].strip()
    completion = original_data["completion"].strip()
...
def old_to_new_file_format():
    dirname = os.path.dirname(__file__)
    inputFilePath = os.path.join(
        dirname, "poems/labels/ft2-modernist-labels-training_5000_prepared_train.jsonl"
    )
    outputFilePath = os.path.join(dirname, "poems/labels/ft2-labels-5000_train.jsonl")
    with (
        open(inputFilePath, "r", encoding="utf-8") as input_file,
        open(outputFilePath, "w", encoding="utf-8") as output_file,
    ):
        i = 0
...
def upload_file(filename):
    dirname = os.path.dirname(__file__)
    inputFilePath = os.path.join(dirname, filename)
    with open(inputFilePath, "rb") as input_file:
        try:
            response = ai.files.create(file=input_file, purpose="fine-tune")
            print("File is valid and uploaded successfully.")
...
def createFineTune():
    ai.fine_tuning.jobs.create(
        validation_file="file-JdXQmY2ya9UYtrscMR4FJ9",
        training_file="file-GTDAvFUrDjbKGEkyTyq6Ab",
...
if __name__ == "__main__":
    ...
    # old_to_new_file_format()
    # upload_file("poems/labels/ft2-labels-5000_train.jsonl")
    createFineTune()
